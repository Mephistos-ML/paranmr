...
"""Integration tests for Hungarian assignment in the ``fit_susc`` workflow.

These tests verify that the Hungarian assignment algorithm produces
susceptibility tensor results equivalent to those from the exhaustive
permutation method.

Test matrix:
    - ``diag_equal``: diagonal chi tensor, equal number of experimental peaks
      and nuclei
    - ``diag_fewer``: diagonal chi tensor, fewer peaks than nuclei
      (underdetermined)
    - ``full_equal``: full chi tensor, equal number of peaks and nuclei
    - ``full_fewer``: full chi tensor, fewer peaks than nuclei
...
def parse_susceptibility_csv(csv_path: Path) -> dict[float, dict[str, float]]:
    """Parse a susceptibility tensor CSV into per-temperature results.

    Args:
        csv_path: Path to the ``susceptibility_tensor.csv`` output file.

    Returns:
        A mapping from temperature to chi tensor parameters.
    ...
    for row in reader:
        ...
        try:
            ...
            chi_iso_key = [k for k in row if "chi_iso" in k and "s-dev" not in k][0]
            chi_ax_key = [k for k in row if "chi_ax" in k and "s-dev" not in k][0]
            chi_rho_key = [k for k in row if "chi_rho" in k and "s-dev" not in k][0]
...
def run_fit_susc(
    ...
    """Run ``fit_susc`` with a specific assignment method.

    Args:
        test_data_dir: Directory containing ``input.yml`` and data files.
        temp_dir: Temporary directory for copied inputs and generated outputs.
        method: Assignment method to use. Must be either ``"permute"`` or
            ``"hungarian"``.
        single_temp: Specific temperature to retain in the copied experimental
            CSV files. If ``None``, keep all temperatures.

    Returns:
        Susceptibility tensor parameters mapped by temperature.
    """
...
def get_assignment_from_output(temp_dir: Path, temp: float) -> list[str]:
    """Extract the assignment list for a specific temperature from output CSVs.

    This helper supports two output formats:

    - ``assigned_experiment_*.csv`` produced by the permute method
    - ``hyperfines_and_fitted_shifts_*.csv`` produced by the Hungarian method

    Args:
        temp_dir: Output directory containing assignment CSV files.
        temp: Temperature for which to retrieve the assignment.

    Returns:
        A list of assigned chemical labels, or ``None`` if no assignment file
        or assignment data can be found.
    """
    ...
    if not assigned_csv:
        ...
        assigned_csv = list(
            temp_dir.rglob(f"hyperfines_and_fitted_shifts_{temp:.2f}_K.csv")
        )
...
def test_hungarian_vs_permute_diag_equal(tmp_path):
    ...
    When the number of experimental signals equals the number of
    ``chem_label`` groups, both the Hungarian algorithm and the exhaustive
    permutation search solve the same assignment problem. They should either
    find identical assignments or produce different but effectively
    degenerate assignments with nearly identical chi tensors.

    Raises:
        AssertionError: Raised if the Hungarian assignment quality is
            significantly worse than the permute result, or if the fitted chi
            tensors differ beyond the allowed tolerance.
    """
...
def test_hungarian_r2_quality_diag_equal(tmp_path):
    """Check that Hungarian fitting reaches the minimum acceptable R².

    This test runs the Hungarian assignment method on the ``diag_equal``
    dataset and verifies that every fitted temperature reaches at least the
    configured R² threshold. It guards against silent degradation where the
    method returns a stable but suboptimal assignment.

    Raises:
        AssertionError: Raised if any fitted temperature has an R² value below
            ``_R2_THRESHOLD``.
    """
    test_data = Path("tests/integration/test_data/hungarian_assignment/diag_equal")
    ...
    for temp in sorted(chi_hungarian.keys()):
        ...
        if r2 < _R2_THRESHOLD:
            r2_failures.append(f"T={temp}K: R²={r2:.6f} < threshold {_R2_THRESHOLD}")
