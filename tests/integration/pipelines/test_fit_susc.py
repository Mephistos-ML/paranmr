...
def _remove_generated_project(directory: Path, project_name: str) -> None:
    """Remove copied fit output so the CLI result is generated by this test run."""
...
def test_fit_susc_moments_gmm_recovers_dyl1_reference_proton_partition(
    ...
    _remove_generated_project(gmm_cwd, "DyL1_1H_Fitting_Moments_iso_ax_rho")
...
def test_fit_susc_moments_gmm_recovers_ybl8_reference_proton_partition(
    ...
    _remove_generated_project(gmm_cwd, "YbL8_1H_PD_fit_Moments")
    ...
    reference_group_sizes = [len(group) for group in expected_groups]
    recovered_partition = _partition_protons_by_sorted_shift(
        hyperfines_csv=gmm_hyperfines_csv,
        group_sizes=reference_group_sizes,
    )
    failures: list[str] = []
    if recovered_partition != expected_partition:
        failures.append("GMM sorted-shift proton partition does not match reference")
    ...
    for proton_group in expected_groups:
        delta_ppm = abs(gmm_centers[proton_group] - fixed_centers[proton_group])
        if delta_ppm >= ppm_tolerance:
            failures.append(
                f"{sorted(proton_group)} center differs by {delta_ppm:.6f} ppm "
                f"(tolerance {ppm_tolerance:.6f} ppm)"
            )
    ...
    if not np.isfinite(fixed_chi_ax):
        failures.append("Fixed-reference chi_ax is not finite")
    if not np.isfinite(gmm_chi_ax):
        failures.append("GMM chi_ax is not finite")
    elif abs(gmm_chi_ax - fixed_chi_ax) >= 0.01:
        failures.append(
            f"chi_ax differs by {abs(gmm_chi_ax - fixed_chi_ax):.6f} "
            "A^3 (tolerance 0.010000 A^3)"
        )

    assert not failures, "YbL8 GMM validation failures:\n" + "\n".join(failures)
