...
def load_diamagnetic_shifts(
    ...
    if file_type == "csv":
        ...
        if has_atom_labels and has_signal_labels:
            raise ValueError(
                "Diamagnetic shift CSV must contain exactly one of "
                "'atom_label' or 'signal_label'"
            )
        if has_atom_labels:
            key_kind = "atom_label"
            dia_by_key = {
            ...
            raise KeyError(
                "Diamagnetic shift CSV requires 'atom_label' or 'signal_label'"
            )
