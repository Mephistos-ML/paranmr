...
``simpnmr_x``. It is intended for developers who want to understand how the
...
The architecture of ``simpnmr_x`` is guided by the following principles:
...
1. **CLI layer** (``simpnmr_x.cli``)
2. **Application layer** (``simpnmr_x.app``: pipelines, loaders, policies)
3. **Domain / core layer** (``simpnmr_x.core``)
4. **IO layer** (``simpnmr_x.io``)
5. **Visualisation layer** (``simpnmr_x.viz``)
...
.. code-block:: text
   ...
   ├── simpnmr_x/             # Python package source
   └── tests/               # Test suite

The remainder of this page focuses on the ``simpnmr_x`` package itself.


Package overview
----------------

``simpnmr_x`` is organised as follows:
...
Application layer (``simpnmr_x.app``)
-----------------------------------
...
- ``loaders``: application-layer adapters that translate user configuration into
  ...
  options, delegate file parsing to ``simpnmr_x.io``, and delegate scientific
  assembly to ``simpnmr_x.core.build``.
...
**Constraint:** numerical algorithms must not be implemented in ``simpnmr_x.app``.
Any scientific formulas, optimisation logic, scoring/metrics that affect results,
or numerical kernels belong in ``simpnmr_x.core``.


Core domain layer (``simpnmr_x.core``)
------------------------------------
...
vice versa), leaving scientific assembly to ``simpnmr_x.core.build``.


Visualisation layer (``simpnmr_x.viz``)
-------------------------------------
...
infrastructure (``simpnmr_x.viz``) rather than ad-hoc Matplotlib code.
...
- adding support for a new QC output format → ``simpnmr_x.io``
- introducing a new susceptibility model → ``simpnmr_x.core``
- adding a new workflow or CLI command → ``simpnmr_x.app`` and ``simpnmr_x.cli``
...
- Application policy (e.g. default selection rules, priority ordering, legacy overrides) must live in ``simpnmr_x.app.policies`` and must not be duplicated across loaders or pipelines.
- Scientific assembly of parsed data into canonical domain entities must live in
  ``simpnmr_x.core.build``. Loaders and pipelines may orchestrate builder calls
...
- Numerical algorithms must not be implemented in ``simpnmr_x.app``. Scientific
  formulas, optimisation logic, scoring/metrics that affect results, or
  numerical kernels belong in ``simpnmr_x.core``.

- All new plots must use the unified visualisation system in ``simpnmr_x.viz``.
  Ad-hoc plotting logic in pipelines or tools is not permitted.

- Experimental or exploratory code must live in ``simpnmr_x.tools`` (or in
...
- Dependency boundaries must be respected. The core layer should not introduce
  ...
  scientifically justified and kept out of ``simpnmr_x.core`` whenever possible.
