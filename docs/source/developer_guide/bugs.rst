...
If you encounter unexpected behaviour or believe you have found a bug in ``simpnmr_x``, we appreciate you letting us know – this helps us improve the code for everyone.
...
1. Make sure you are using the latest version of ``simpnmr_x``:

   .. code-block:: bash

       pip install --upgrade simpnmr_x
...
If the problem persists and is not addressed in the FAQ, please open a `GitHub Issue <https://github.com/Mephistos-ML/simpnmr_x/issues>`_
...
1. Versions of ``Python`` and ``simpnmr_x`` (for example, from ``python --version`` and ``pip show simpnmr-x``).
2. Your operating system and how ``simpnmr_x`` was installed (e.g. system Python, virtual environment, conda environment).
...
5. The full error message and/or any unexpected output produced by ``simpnmr_x``.
