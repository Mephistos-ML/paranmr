...
If you encounter unexpected behaviour or believe you have found a bug in ``paranmr``, we appreciate you letting us know – this helps us improve the code for everyone.
...
1. Make sure you are using the latest version of ``paranmr``:

   .. code-block:: bash

       pip install --upgrade paranmr
...
1. Versions of ``Python`` and ``paranmr`` (for example, from ``python --version`` and ``pip show paranmr``).
2. Your operating system and how ``paranmr`` was installed (e.g. system Python, virtual environment, conda environment).
...
5. The full error message and/or any unexpected output produced by ``paranmr``.
