...
Below, we outline the key models and equations implemented in ``simpnmr_x``.
...
In ``simpnmr_x``, the diamagnetic, spin-dipolar, Fermi-contact,
...
shift. In ``simpnmr_x``, the diamagnetic shift contribution is written as
...
In ``simpnmr_x``, the contribution arising from the anisotropic magnetic
...
In ``simpnmr_x``, this defines the spin-dipolar contribution independently of
...
In ``simpnmr_x``, :math:`\delta^{\mathrm{FC}}` is evaluated from the isotropic
...
anisotropy on the FC term, ``simpnmr_x`` treats the corresponding correction as
...
In ``simpnmr_x``, the orbital contribution is treated as two additional shift
...
.. note::
   ...
   ``simpnmr_x``, :math:`\mathbf{A}^{\mathrm{SD}}` still defines the
