...
We welcome contributions to ``SimpNMR``. If you would like to add functionality or improve the code or documentation,
please start by creating an `Issue <https://gitlab.com/suturina-group/simpnmr/-/issues>`_ on GitLab (using the relevant template)
to describe the change you are proposing.

When contributing, you **must** follow the rules below, which help to standardise development and keep the project maintainable.
...
To build the documentation **locally**, from the repository root navigate to the ``docs`` directory and install the Python dependencies with:
...
To publish the documentation online, create a GitLab pipeline with the keyword ``force`` and the value ``docs`` so that the documentation job is triggered.