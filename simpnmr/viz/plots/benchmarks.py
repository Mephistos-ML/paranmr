...
def plot_a_fc_spread(
    ...
    *,
    ...
    datasets = [
        [float(entry["a_fc"]) for entry in chem_label_summary[label]["values"]]
    ...
    ax.set_ylabel(r"$A_\mathregular{FC}$ (ppm Å$^\mathregular{-3}$)")
...
def plot_a_fc_functional_max_curve(
    ...
    *,
    ...
    ax.set_ylabel(r"max $A_\mathregular{FC}$ (ppm Å$^\mathregular{-3}$)")
    ax.set_title(f"{formatted_nucleus} max $A_\\mathregular{{FC}}$")
