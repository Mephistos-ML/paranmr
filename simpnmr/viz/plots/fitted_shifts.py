...
def _draw_fitted_shift_labels(
    ...
    *,
    ...
    """Build and draw fitted-shift labels.
        ...
        if average:
            ...
            for calc_value, exp_value in zip(calc, expt):
                label_texts.append(
                    ax.annotate(
                        label,
                        (float(calc_value), float(exp_value)),
                        xytext=(3.5, 3.5),
                        textcoords="offset points",
                        fontsize=fontsize,
                    )
...
def _draw_fitted_shift_header_table(
    header_ax: plt.Axes,
    molecule: Molecule,
...
def plot_fitted_shifts(
    ...
    _draw_fitted_shift_header_table(
        header_ax,
        molecule,
        susc_model,
        scale=scale,
